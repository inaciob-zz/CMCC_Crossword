...
function oyBrowserDetection(){
	...
	//This section NEEDS TO BE UPDATED CONSTANTLY to comply with modern browsers.
	...
	if (
		(this.isIE && this.version>=7) || 
		(this.isFirefox && this.version >=18) || 
		(this.isChrome && this.version >=23)
	){	
		this.isGood = true;	
	} 
}
...
//The versions listed here must match the baselines set directly above for consistency. 
function oygBind(puzz){
	...
	if (!isSupported){  
		var msg =  
			...
			"We support Microsoft IE 7.0+, Firefox 18.0+, and Chrome 23.0+ versions.\n\n";
		if (confirm(msg)){  
			isSupported = true;
		}
	} 
