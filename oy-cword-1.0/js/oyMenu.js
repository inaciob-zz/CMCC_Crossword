...
function oyCrosswordMenu(puzz){
	...
	this.scoreSubmittedMatches = 0;	// number of matches for which their score was submitted successfully
} 
...
oyCrosswordMenu.prototype.leaveGameEarly = function(url){
	...
	if (this.puzz.started && !this.over){
		canLeave = confirm("Game is in progress. Do you really want to leave the game?");
	}	  
	if (canLeave){ 
		window.location = url;
	}
...
oyCrosswordMenu.prototype.askNickName = function(score){
	...
	this.name = window.prompt(  
		score + "Please enter your KIRO username.\n" +  
		"Without any credentials, your score will not be recorded.",
		this.name 
	);
...
oyCrosswordMenu.prototype.revealWord = function(clue){
	...
	this.footer.stateOk("Revealed " + clue.answer.toUpperCase() + ".");
}  
...
oyCrosswordMenu.prototype.checkAll = function(){
	...
	if  (checked == 0){
		...
		this.footer.stateOk(correct + " words have been matched!");
	}
}  
...
oyCrosswordMenu.prototype.checkWord = function(clue){
	var status = this.checkWordStatus(clue);	  
	if (!status.isComplete){
		this.footer.stateError("The word " + status.buf + " is either incorrect or incomplete!");
	} else { 
		this.checks++; 
		this.deducts += this.getDeductionForCheck(clue);			
		if (status.wrong != 0){		  
			//this.footer.stateError("Sorry! " + status.buf + " didn't match!");
			this.footer.stateError("Sorry!, one or more of your words didn't match!");
		} else { 
			this.matches++; 
			...
			//this.footer.stateOk("Great! " + status.buf + " matched!");
			this.footer.stateOk("Great! your word(s) matched!");
		}
	}
}
...
oyCrosswordMenu.prototype.submitScore = function(){
	if (this.matches == 0){   
		...
		alert("Nothing to submit yet!\nTry to uncover some words first.");
	} else {		  
